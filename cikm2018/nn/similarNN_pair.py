...
class Model:
    def __init__(self, word_emb_dim, rnn_hid_dim, rnn_n_layers, max_sen_length, learning_rate, keep_prob
                 , vocab_size_1, vocab_size_2, n_class, class_weights, word2id_1, word2id_2, label2id):
        ...
        self.vocab_sze_1 = vocab_size_1
        self.vocab_sze_2 = vocab_size_2
        ...
        self.word2id_1  = word2id_1
        self.word2id_2  = word2id_2
    ...
    def build(self, build_session = True, init_word_embedding_1=None, init_word_embedding_2=None):
        ...
        """
        self.emb_1 = self.__build_embedding(scope='word_embedding_1', reuse=False, vocab_size=self.vocab_sze_1, word_emb_dim=self.word_emb_dim,
                                            input=self.input1)
        self.emb_2 = self.__build_embedding(scope='word_embedding_2', reuse=False, vocab_size=self.vocab_sze_2, word_emb_dim=self.word_emb_dim,
                                            input=self.input2)

        """
        building biGRU
        """
        (self.output_fw1, self.states_fw1), (self.output_bw1, self.states_bw1) = self.__build_GRU(scope='biGRU_spa',
        ...
        (self.output_fw2, self.states_fw2), (self.output_bw2, self.states_bw2) = self.__build_GRU(scope='biGRU_eng',
                                                                                                  rnn_n_layers=self.rnn_n_layers, rnn_hid_dim=self.rnn_hid_dim, keep_prob=self.keep_prob, reuse=False,
        ...
        if build_session:
            ...
            if init_word_embedding_1 is not None:
                word_emb_placeholder_1 = tf.placeholder(dtype=tf.float32, shape=init_word_embedding_1.shape, name='word_emb_placeholder_1')
                word_emb_placeholder_2 = tf.placeholder(dtype=tf.float32, shape=init_word_embedding_2.shape, name='word_emb_placeholder_2')

                assign_op_1 = tf.assign(self.sess.graph.get_tensor_by_name(name='word_embedding_1/E:0'), word_emb_placeholder_1)
                assign_op_2 = tf.assign(self.sess.graph.get_tensor_by_name(name='word_embedding_2/E:0'), word_emb_placeholder_2)

                self.sess.run([assign_op_1,assign_op_2], feed_dict={word_emb_placeholder_1: init_word_embedding_1,
                                                                    word_emb_placeholder_2: init_word_embedding_2})
